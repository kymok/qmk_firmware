...
static const pin_t row_pins[] = {D6, D7, B4, B5};
static const pin_t col_pins[] = {B6, C6, C7, D2, D3, D5};
const int CUSTOM_MATRIX_ROWS = 4;
const int CUSTOM_MATRIX_COLS = 6;
...
bool matrix_scan_custom(matrix_row_t current_matrix[]) {
    ...
    for (uint8_t current_row = 0; current_row < CUSTOM_MATRIX_ROWS; current_row++) {
        matrix_has_changed |= read_cols_on_row(raw_matrix, current_row, CUSTOM_MATRIX_ROWS);
    }

    // Read rows
    init_pins();
        for (uint8_t current_col = 0; current_col < CUSTOM_MATRIX_COLS; current_col++) {
        matrix_has_changed |= read_rows_on_col(raw_matrix, current_col, 0);
    }
